...
function Show-Usage {
    @"
sprout $SproutVersion - grow a new project
...
switch ($Name) {
    ...
    '--version'               { Write-Host "sprout $SproutVersion"; return }
    '--langs'                 { Write-Host ($SupportedLangs -join ' '); return }
}

if ([string]::IsNullOrWhiteSpace($Name)) {
    Write-Error "You must provide a project name.`nUsage: sprout <project-name> [language]"
    exit 1
}
...
> Generated with [sprout]($SproutRepo) — scaffolds a ready-to-run project and opens it in your editor.
...
function Initialize-Git {
    param([string]$Lang)
    if ($env:SPROUT_NO_GIT -eq '1') { return }
...
if ($EditorCmd -eq 'none') {
    ...
    Write-Host "Set SPROUT_EDITOR to your editor (e.g. 'code') or 'none'."
}
